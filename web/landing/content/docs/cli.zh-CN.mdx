---
title: 命令行工具
description: termio 命令——从 shell 打开项目，以及让你或另一个 Agent 派生、驱动、监督会话的 termio sessions 编排 API。
x-i18n:
  source_path: cli.mdx
  source_hash: 594a99d427b865712f7cb6faa8cd459e040b54d0ba11b7e09db266c61f376f70
  generated_at: 2026-09-17
---

`termio` 是这个应用在 shell 一侧的入口：它打开项目，而通过 `termio sessions`，它就是
一个 Agent 用来查看和驱动兄弟会话的编排 API。开关在
[会话控制](/docs/session-control)。

打开 **设置 ▸ Termio server ▸ 命令行 ▸ 命令行工具**。它会把一个 `termio` 命令软链到你的 `PATH` 上
