...
- **Stable handles, one target.** Sessions are addressed by `<agent>@<id>`
  handles minted at creation and printed by `list`. The agent name is a
  checksum: a name spliced onto the wrong id is an error, not a misdelivery.
