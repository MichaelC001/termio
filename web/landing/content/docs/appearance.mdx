...
- **Agents** — add agents, enable them, and turn on [session
  control](/docs/session-control). Values that differ per machine — where a CLI
  lives, whether it’s there — are read on that machine’s own page instead; see
...
A machine is chosen by navigation — **Server** for this Mac, a row under
**Remote Hosts** for any other — never by a picker on a page about something
else. What you change on a machine’s page is what that machine gets.
