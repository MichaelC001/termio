...
iPhone companion are all *viewers*. A viewer attaches to a session and detaches
...
| The server stops, or the machine reboots | No | Yes | No | Only where the agent supports [resume](/docs/atp#resume) |
| **Close Session** (`⌘W`) | No | — | — | — |

The top six rows are all the same event wearing different clothes: a viewer went
away and the process didn’t notice. Reattaching repaints the screen from the
...
<Callout type="note">
  ...
  inspector tab you left each workspace on. Termio writes that structure to disk;
  it never writes the contents of your terminals.
</Callout>
...
- **Sessions restart in their own directory.** A shell comes back as a shell,
  rooted where it was.
- **Agents that can resume, do.** Reopening a session relaunches the agent into
...
With a service installed the server comes back after a crash, a logout, and a
reboot. This is the single most common fix for “my remote sessions vanished” —
...
- **Termio doesn’t record your terminals.** Scrollback lives in the server for
  the session’s lifetime and goes away with it. There is no transcript of your
  shells written to disk.
