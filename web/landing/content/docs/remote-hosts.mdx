...
- **Add Host…** — at the end of that submenu, and on the Remote Hosts pane —
  appends a `Host` block for a box that isn’t in your config yet. Lead with the
...
it is a **Test** button, and it has four answers:

| Answer | What it means |
| --- | --- |
| Reachable | `ssh` connected and authenticated. Nothing to fix. |
| Wants a password | The host offered password auth. This is the one failure with a one-click fix — see below. |
| Auth failed | It answered, and refused the credentials. A key problem, not a network one. |
| Unreachable | Nothing answered. Wrong address, box asleep, no route. |
...
That isn’t a policy choice — it’s arithmetic. The connections that carry
sessions, the file tree, search, and the git panes all run non-interactively
...
| **Ready** | Agents on that machine can run — and report their status back here. |
...
SSH, probe for your agent CLIs, then install the hooks and the skill into each
agent’s own config. No root, no package manager, nothing added to your shell
profile.
...
- **Installed by Termio.** What Termio wrote into each agent’s config on that
  machine — **Hooks** (so agents report their status) and the **Skill** (so they
  know the `termio sessions` commands exist). One **Reinstall Hooks and Skill**
  button repairs both; reach for it after hand-editing one, or when a status dot
  stops moving.
