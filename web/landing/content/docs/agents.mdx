...
doing right now. The point isn’t that you can open several terminals — any
terminal does that. It’s that you can stop watching them.
...
terminal title; **screen** is pattern-matching on the pane, used only when there
is nothing better.
...
own flag when it launches. It’s per agent and per machine, off by default, and it
changes nothing about what the agent can reach — it’s the vendor’s flag, not a
Termio sandbox.
