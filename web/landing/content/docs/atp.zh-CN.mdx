...
x-i18n:
  source_path: atp.mdx
  source_hash: 2f51dccdbf356ac52a0ae23bb27d5c9873147318bf9d9b651be00ee664354485
...
| `skills` | Agent 的用户级技能目录（`{"dir": "~/.claude/skills"}`），Termio 的会话控制技能就写在那儿。可选；没有技能生态的 Agent 可以不写。 |
| `configHome` | Agent 自己文档里那个用来搬走配置树的环境变量，以及它替换掉的路径（`{"env": "CLAUDE_CONFIG_DIR", "path": "~/.claude"}`）。写上它，Termio 就会装到 Agent 真正读取的位置，而不是那个字面默认值。可选。 |
