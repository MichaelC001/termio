---
title: Concepts
description: The nouns Termio is built from — machine, workspace, project, session, pane, and worktree — plus the status model that tells you which agent needs you.
---
...
A machine is a box you can reach: this Mac, the Mac mini on the same desk, a VPS.
...
a second one. See [Workspaces and machines](/docs/machines).
...
`⌘1` through `⌘9` switch between them.
...
- **A session lives on its machine, not in the app.** Every session — including
  ...
  **Close Session** ends one on purpose.
