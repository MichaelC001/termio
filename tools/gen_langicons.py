#!/usr/bin/env python3
"""Download Devicon colored SVGs for the file types termio's tree shows, and emit
a Swift catalog mapping extension / filename -> icon resource name.

Run from the repo root. Writes SVGs into Sources/termio/LangIcons/ and the
catalog into Sources/termio/FileBrowser/LangIconCatalog.swift.
"""
import json, os, sys, urllib.request, concurrent.futures, re

REPO = os.path.abspath(os.path.dirname(__file__))  # overridden below
REPO = "/Users/yuanjiwei/Documents/GitHub/termio"
ICON_DIR = os.path.join(REPO, "Sources/termio/LangIcons")
CATALOG = os.path.join(REPO, "Sources/termio/FileBrowser/LangIconCatalog.swift")
...
def main():
    ...
    lines.append("// Maps a file's extension or exact name to a bundled Devicon SVG")
    lines.append("// (Sources/termio/LangIcons/<name>.svg). Devicon is MIT-licensed;")
    ...
    lines.append("enum LangIconCatalog {")
    ...
    lines.append("    static let byName: [String: String] = [")
    ...
    lines.append("    static let byExtension: [String: String] = [")
    ...
    lines.append("    static let monochrome: Set<String> = [")
