...
Align with [session-daemon-architecture](session-daemon-architecture.md):
...
- Mirror host events into per-session `@Observable SessionRuntime` — see [sidebar-scroll-performance](sidebar-scroll-performance.md).
...
- Sidebar remains scroll-smooth with 10 local + 10 remote working agents ([sidebar-scroll-performance](sidebar-scroll-performance.md) verification upgraded).
...
| Doc | Relation |
| --- | --- |
| [session-daemon-architecture.md](session-daemon-architecture.md) | Parent host model; **authoritative terminal state** — this doc adds product/competitive/protocol staging |
| [sidebar-scroll-performance.md](sidebar-scroll-performance.md) | **Client observation** contract for host event volume |
| [remote-projects.md](remote-projects.md) | Interim SSH path; superseded by Phase 2 |
| [remote-access-relay-strategy.md](remote-access-relay-strategy.md) | Phone WSS transport |
| [worktree-creation-lifecycle.md](worktree-creation-lifecycle.md) | Local worktrees; remote creation runs **on termiod host** |
