...
**Evidence policy** (inherited from [termiod-session-protocol.md](termiod-session-protocol.md)):
...
2. **Route selection.** With several routes reachable, choose by latency —
   but probing costs, and it is unresolved whether a live session migrates
...
- **Supersedes** §C.6's "sidecar only" framing of the host VT (§3), and the
  local/remote asymmetry in [remote-projects.md](remote-projects.md).
- **Extends** [termiod-session-protocol.md](termiod-session-protocol.md) with
  the device layer above the host noun; §C.10's resumable subscription becomes
  the recovery rule for all four planes.
- **Realises** [session-daemon-architecture.md](session-daemon-architecture.md)'s
