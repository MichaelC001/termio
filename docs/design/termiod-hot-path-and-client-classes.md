...
related:
  ...
  - termiod-device-architecture.md
  - termiod-vt-sidecar-spike.md
---
...
[termiod-session-protocol.md](termiod-session-protocol.md)): Superlogical
...
- **`session_id` is on the wire, on disk, and in the CLI.** Renaming touches
  ...
  `session-deep-link.md`'s URL scheme. That is a `proto:2` break for a synonym.
...
## E. Protocol deltas vs `termiod-session-protocol.md`
