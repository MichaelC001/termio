...
| 每 issue 隔离环境 | 现搭 worktree | **worktree 创建 + 3 级侧栏 UI** 已在（见 `worktree-creation-lifecycle.md`） |
| 公网入站隧道 | 装 Hookdeck/cloudflared | **companion 的 cloudflared named tunnel 已常驻**（给手机用），加一条 HTTP route 即可（见 `remote-access-relay-strategy.md`） |
| 多 agent | 只有 Claude Code | **Claude/Codex/Pi/Grok** 都能起，ATP manifest 已抽象好（见 `agent-abstraction-and-configuration.md`） |
...
- 无匹配规则 = 静默丢弃（收端丢陌生事件，见 `session-daemon-architecture.md` 的既有做法）。
...
1. **Mac 睡眠是硬约束。** 合盖时本地 agent 根本没法跑（见 `remote-access-lessons.md` / mac-reachable 结论：第三方 app 无法可靠唤醒睡眠的 Mac；Cyrus 也只能靠 tmux/pm2/systemd 保活）。这决定产品语气二选一：
...
- **身份**：Linear `actor=app` 需要注册一个 Termio OAuth app —— 是每个用户自建（Cyrus 社区版模式），还是 Termio 托管一个共享 app（省用户事，但 Termio 要跑网络层）？关系到 `remote-access-relay-strategy.md` 里 BYO-tunnel vs 托管 relay 的同一取舍。
