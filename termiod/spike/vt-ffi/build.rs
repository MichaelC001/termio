...
fn zig_target(target: &str) -> &str {
    match target {
        ...
        other => panic!("unsupported libghostty-vt spike target: {other}"),
    }
}
...
fn generate_bindings(vendor: &Path, out_dir: &Path, host: &str) {
    ...
    let bindings = bindgen::Builder::default()
        ...
        // Bindgen runs on the host. Ghostty's public header is target-neutral and both supported
        // spike targets have a 64-bit size_t; forcing HOST avoids requiring a musl C sysroot just
        // to parse stddef.h while Zig independently builds the target archive.
...
fn main() {
    ...
    if target.contains("apple-darwin") {
        // Match Herdr: pass the fat archive directly on Darwin.
